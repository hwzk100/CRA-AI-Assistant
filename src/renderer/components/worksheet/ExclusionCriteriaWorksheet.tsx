 ...
 * Same as inclusion but with inverted eligibility colors
 */

import React, { useState } from 'react';
import { useExclusionCriteria, useStore, useSubjectFiles } from '../../hooks/useStore';
import type { ExclusionCriteria } from '@shared/types/worksheet';
...
export const ExclusionCriteriaWorksheet: React.FC = () => {
  ...
  const handleEdit = (criteria: ExclusionCriteria) => {
    ...
    setEditForm({ category: criteria.category, description: criteria.description });
  };

  const handleSave = () => {
    if (editingId) {
      updateExclusionCriteria(editingId, { ...editForm, updatedAt: new Date() });
    }
    setEditingId(null);
  ...
  const handleDelete = (id: string) => {
    if (confirm('确定要删除这条排除标准吗？')) removeExclusionCriteria(id);
  };
  ...
  const getMergedResult = (criteria: ExclusionCriteria) => {
    ...
    if (criteria.fileResults && criteria.fileResults.length > 0) {
      ...
      if (allEligible) return { eligible: true, reason: `所有文件均符合：${fileResults.map(r => r.fileName).join('、')}` };
      if (allIneligible) return { eligible: false, reason: `所有文件均不符合：${fileResults.map(r => r.fileName).join('、')}` };
      ...
      <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          {hasAnalyzedData && (
            <button
              onClick={() => {
                if (confirm('确定要清除所有分析结果吗？')) clearExclusionEligibility();
              }}
                  ...
                  {criteria.map((c) => (
                    <div
                      ...
                      className={`bg-white border rounded-lg p-3 hover:border-gray-300 transition-colors ${
                        c.eligible === false ? 'border-green-300 bg-green-50' :
                        c.eligible === true ? 'border-red-300 bg-red-50' :
                        'border-gray-200'
                      }`}
                    >
                      {editingId === c.id ? (
                        <div className="space-y-2">
                          <input type="text" value={editForm.category} onChange={(e) => setEditForm({ ...editForm, category: e.target.value })} placeholder="分类" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent text-sm" />
                          <textarea value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} placeholder="标准描述" rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent text-sm resize-none" />
                          <div className="flex items-center space-x-2">
                                ...
                                return (
                                  <div className="flex items-center space-x-2 mt-2">
                                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                                      mergedResult.eligible ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                                    }`}>
        ...
        {isAdding && (
          <div className="fixed bottom-0 right-0 w-96 bg-white border-t border-gray-200 shadow-lg p-4">
            ...
            <div className="space-y-3">
              <input type="text" value={editForm.category} onChange={(e) => setEditForm({ ...editForm, category: e.target.value })} placeholder="分类" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
              <textarea value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} placeholder="标准描述" rows={4} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-none" />
              <div className="flex items-center space-x-2">
                <button onClick={handleAdd} disabled={!editForm.description} className="flex-1 px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">添加</button>
                <button onClick={handleCancel} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">取消</button>
              </div>
            </div>
          </div>
        )}
